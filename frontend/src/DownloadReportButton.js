...
const DownloadReportButton = ({ results, targetUrl, aiThreats }) => {
  ...
  const handleDownload = () => {
    ...
    setTimeout(() => {
      try {
        ...
        doc.text("Security Scan Report", margin, 60);
        ...
        if (results?.whatweb_info?.length > 0) {
          ...
          addTableSection("1. Target URL information", ["Target", "Server", "IP"], wwBody);
        }
        ...
        // --- 2. ENUMERATION (Subdomains & Directories) ---
        
        // 2a. Sublist3r (Subdomains)
        if (results?.sublist3r_info && results.sublist3r_info.length > 0) {
          const subBody = results.sublist3r_info.map(s => [s.subdomain]);
          addTableSection("2. Discovered Subdomains", ["Subdomain"], subBody);
        }

        // 2b. Gobuster (Directories)
        if (results?.gobuster_info && results.gobuster_info.length > 0) {
          // Note: The backend still stores the path in the 'subdomain' field key
          const dirBody = results.gobuster_info.map(s => [s.subdomain]);
          addTableSection("2.1 Discovered Directories", ["Path"], dirBody);
        }
  ...
  (p.reason === 'no-response' ? '--' : (p.reason || 'N/A'))
            ...
            addTableSection("3.Ports Details", ["Host IP", "Port", "Protocol", "State", "Service","Reason"], portBody);
          }
          ...
          if (uniqueCiphers.length > 0) {
            ...
            addTableSection("3.1 SSL Ciphers Details", ["Cipher Name", "Strength"], cipherBody);
          }
        }
        ...
        if (wapitiVulns.length > 0) {
          ...
          addTableSection("4. Discovered Vulnerabilities", ["Category / Info"], wBody);
        }
        ...
        if (skipfishVulns.length > 0) {
          ...
          addTableSection("5.Issues in Affected URL", ["Severity", "Description", "Affected URL"], sBody);
        }
        ...
        if (sqlmapVulns.length > 0) {
          ...
          addTableSection("6. SQL Injection Findings", ["Parameter", "Injection Type", "Title"], sqlBody);
        }


         // --- 7. AI EXECUTIVE SUMMARY (Now uses Narrative format!) ---
        if (aiThreats && aiThreats.length > 0) {
          addNarrativeSection("7. Vulnerability Analysis", aiThreats);
        }
        ...
        if (results?.metasploit_info?.report && Array.isArray(results.metasploit_info.report)) {
          addNarrativeSection("8. Metasploit Exploitation", results.metasploit_info.report);
        }
