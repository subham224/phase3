...
async def run_nmap_scans(target_url: str, scan_type: ScanType, scan_id: str, update_progress: callable, command_increment: float, timestamp: str) -> Tuple[Dict[str, Any], Optional[str]]:
    nmap_results = {}
    target_list_path = NMAP_TARGETS_FILE
    ai_output_file = os.path.join(SCAN_OUTPUT_DIR, f"ai_nmap_output_{scan_id}_{timestamp}.json")
    
    # Extract just the domain or IP from the full target_url (Nmap doesn't like http:// prefixes)
    parsed_url = urlparse(target_url)
    target_domain = parsed_url.netloc.split(':')[0] if parsed_url.netloc else target_url.split('/')[0]
    ...
    if scan_type == ScanType.LIGHT:
        commands_to_run.extend([
            ["nmap", "-p", uports, "-sU", "-sV", "-O", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap1_udp_svc_os_light.xml")],
            ["nmap", "-p", tports, "-sT", "-sV", "-O", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap2_tcp_svc_os_light.xml")],
            ["nmap", "-sO", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap3_ip_protocol_light.xml")],
            ["nmap", "--script", "ssh-auth-methods", "-p", "22", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap4_ssh_auth_light.xml")],
            # ONLY this command uses the target list (-iL)
            ...
            ["nmap", "--script", "http-enum", "-p", "80", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap6_http_enum_light.xml")]
        ])
        ...
        commands_to_run.extend([
            ["nmap", "-p", uports, "-sU", "-sV", "-O", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap1_udp_svc_os_deep.xml")],
            ["nmap", "-p", tports, "-sT", "-sV", "-O", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap2_tcp_svc_os_deep.xml")],
            ["nmap", "-sO", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap3_ip_protocol_deep.xml")],
            ["nmap", "--script", "ssh-auth-methods", "-p", "22", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap4_ssh_auth_deep.xml")],
            # ONLY this command uses the target list (-iL)
            ...
            ["nmap", "--script", "http-enum", "-p", "80", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap6_http_enum_deep.xml")],
            ["nmap", "--script", "vulners", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap7_vulners_deep.xml")],
            ["nmap", "--script", "vuln", "--max-retries", "3", target_domain, "-oX", os.path.join(SCAN_OUTPUT_DIR, "nmap8_vuln_deep.xml")]
        ])
        scan_keys_and_output_files.extend([
